...
def hh(url, city=None, specialization=None):
        ...
        if resp.status_code == 200:
            ...
            for div in div_list:
                ...
                jobs_list.append({
                    'title': v_title.text,
                    'url': v_link,
                    'description': v_desc_resp + ' ' + v_desc_req,
                    'company': company,
...
def superjob(url, city=None, specialization=None):
    headers = header[randint(0, 2)]
    jobs_list = []
    errors = []
            ...
            if not new_jobs:
                ...
                for div in div_list:
                    ...
                    jobs_list.append({
                        'title': v_title.text,
                        'url': v_link,
                        'description': v_desc_resp[:130] + ' ' + v_desc_resp[131:],
                        'company': company,
...
def gorodrabot(url, city=None, specialization=None):
    headers = header[randint(0, 2)]
    jobs_list = []
    errors = []
        ...
        if resp.status_code == 200:
            ...
            for div in div_list:
                ...
                jobs_list.append({
                    'title': v_title.text,
                    'url': v_link,
                    'description': v_desc_resp.strip() + ' ' + v_desc_req,
